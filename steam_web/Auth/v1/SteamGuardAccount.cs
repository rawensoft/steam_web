...
namespace SteamWeb.Auth.v1
{
    public class SteamGuardAccount
    {
        ...
        private async Task<RESPONSE_STATUS> _sendConfirmationAjaxAsync(Confirmation conf, string op)
        {
            ...
            SendConfirmationResponse confResponse = await JsonSerializer.DeserializeAsync<SendConfirmationResponse>(new MemoryStream(Encoding.UTF8.GetBytes(response.Data)));
            return confResponse.Success ? RESPONSE_STATUS.Success : RESPONSE_STATUS.Error;
        }
