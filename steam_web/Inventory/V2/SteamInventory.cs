...
public class SteamInventory
{
    ...
    [JsonPropertyName("context")] public ulong Context { get; set; } = 2;
    [JsonPropertyName("error")] public string? Error { get; init; }
    [JsonPropertyName("is_too_many_requests")] public bool IsTooManyRequests { get; init; } = false;

    public static SteamInventory Load(DefaultRequest defaultRequest, ulong steamid64, uint appid, ulong context = 2, bool trading = false)
    {
    ...
    public async static Task<SteamInventory> LoadAsync(DefaultRequest defaultRequest, ulong steamid64, uint appid, ulong context = 2, bool trading = false)
    {
        string url = GetUrl(steamid64, appid, context, trading);
        var getRequest = new GetRequest(url)
        {
    ...
    public ulong GetContext(uint appId, ulong context = 6)
    {
        if (appId == 753)
            return context;
        return 2;
    ...
    private static string GetUrl(ulong steamid64, uint appid, ulong context, bool trading)
    {
