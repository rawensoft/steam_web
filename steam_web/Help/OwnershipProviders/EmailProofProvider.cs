...
public class EmailProofProvider : IOwnershipProofProvider
{
    ...
    public PROVIDER_LIST Type { get; } = PROVIDER_LIST.Email;
    ...
    public EmailProofProvider(ISessionProvider session, Proxy? proxy, ulong stoken, string referer, string href, Script.Enums.TypeReset reset)
    {
        _session = session;
        _proxy = proxy;
        _stoken = stoken;
    ...
    public VERIFY_STATUS Verify(string code, CancellationToken cancellationToken)
    {
        ...
        OnProviderStatusChanged?.Invoke(Type, true, response.Hash!);
